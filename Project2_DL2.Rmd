...
for(i in 0:frames) {
 ...
 for (j in 1:12){
   ...
   if (i < 10) {name[j] = paste('000',i,'_', j, 'plot.png',sep='')}
   if (i < 100 && i >= 10) {name[j] = paste('00',i,'_', j,'plot.png', sep='')}
   if (i >= 100) {name[j] = paste('0',i,'_', j,'plot.png', sep='')}
 }
 ...
 ggplot(worldmap,aes(long, lat)) +
  ...
  for (j in 1:12){
    print(name[j])
    ggsave(file=paste0("pngs/",(name[j])))
  }
 
}

reduce_command <- 'mogrify -resize 1138x600 *.png'
convert_command <- 'convert *.png -delay 100x100 -loop 0 animation.gif'
setwd("~/Google Drive/Data Science/Courses/Spring 2016/Exploratory Data Analysis and Visualization/Assignments/EDAV_project_2/pngs")
system(reduce_command)
